...
def create_transfer(project_id, transfer_location, dataset, base_name, s3_bucket,
                 ...
                 schedule="every day 00:30", data_path=None):
    """
    Create a single BigQuery transfer configuration for an S3 file.
    ...
    Args:
        ...
        field_delimiter (str): Field delimiter for CSV (default: ;)
        skip_leading_rows (str): Number of rows to skip for CSV (default: 1)
        ...
        schedule (str): Transfer schedule (default: every day 00:30)
        data_path (str): Custom S3 data path (e.g. for wildcard JSONL paths). If None, defaults to s3://bucket/base_name.csv
    ...
    try:
        ...
        if not data_path:
            if is_json:
                data_path = f"s3://{s3_bucket}/{base_name}/*.jsonl"
            else:
        ...
        if not is_json:
            params["field_delimiter"] = field_delimiter
            params["skip_leading_rows"] = skip_leading_rows
...
def setup_transfers(project_id=None, transfer_location=None,
                   ...
                   test_suffix=None, table_filter=None, schedule=None,
                   data_path=None):
    """
    ...
    if not schedule:
        schedule = config.get_transfer_schedule()
    ...
    for i, schema_file in enumerate(schema_files):
        ...
        if create_transfer(project_id, transfer_location, dataset, base_table_name, s3_bucket,
                          ...
                          skip_leading_rows, write_disposition, test_suffix, schedule,
                          data_path):
            success_count += 1
        else:
            error_count += 1
